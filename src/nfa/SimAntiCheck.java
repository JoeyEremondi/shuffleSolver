...
public class SimAntiCheck {
	...
	public enum METHOD
	{
		INITIAL_STATE, SMALL_BUCCHI, FINITE_PREPROCESS, LIGHT_PREPROCESS, DEFAULT;
	}

	public static Pair<Boolean, METHOD> result(Boolean res, METHOD method)
	{
		return new Pair<Boolean, METHOD>(res, method);
	}
	...
	public static Pair<Boolean, METHOD> isSubset( FiniteAutomaton aut1, FiniteAutomaton aut2) {
		...
		if(aut1.F.contains(aut1.getInitialState()) && !aut2.F.contains(aut2.getInitialState())){
			...
			return result(false, METHOD.INITIAL_STATE);
		}
...
/*		if(Options.verbose){
		...
		if(x.result){
			...
			return result(true, METHOD.LIGHT_PREPROCESS);
		}
		...
		if(aut1.states.size()+aut2.states.size() <= 600){
			...
			if(!inclusion.timeout){
				...
				if(inclusion.isIncluded()){
					...
					return result(true, METHOD.SMALL_BUCCHI);
				}else{
					System.out.println("Not Included.");
					return result(false, METHOD.SMALL_BUCCHI);
				}
			}
			/*if(Options.verbose){
		...
		if(x.result){
			...
			return  result(true, METHOD.FINITE_PREPROCESS);
		}
		...
		if(inclusion.isIncluded()){
			//System.out.println("Included.");
			return result(true, METHOD.DEFAULT);
		}else{
			//System.out.println("Not Included.");
			return result(false, METHOD.DEFAULT);
		}
